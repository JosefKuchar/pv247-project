...
export const PlaceReviewsList = ({
  ...
  const loadMore = useCallback(() => {
    ...
    startTransition(async () => {
      try {
        const result = await loadLocationReviewsAction(locationId, page);
  ...
  React.useEffect(() => {
    ...
    const loadInitialReviews = async () => {
      try {
        // Load first page
        const result = await loadLocationReviewsAction(locationId, 1);
        ...
        if (result.items.length < 6 && result.hasMore) {
          const secondResult = await loadLocationReviewsAction(locationId, 2);
  ...
  if (reviewsCount === 0 || reviews.length === 0) {
    return <EmptyReviewsState isOwnProfile={false} userName={placeName} />;
  }
